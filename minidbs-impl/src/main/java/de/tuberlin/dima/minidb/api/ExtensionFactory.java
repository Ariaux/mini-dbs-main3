...
import de.tuberlin.dima.minidb.io.manager.BufferPoolManagerImpl; // 必须导入这个实现类
...
public class ExtensionFactory extends AbstractExtensionFactory {
    ...
    /**
     * Creates the default BufferPoolManager.
     */
    public BufferPoolManager createBufferPoolManager() {
        return createBufferPoolManager(null, null);
    }
    ...
    /**
     * 这里是核心修改：直接实例化 BufferPoolManagerImpl。
     * 逻辑委托给 BufferPoolManagerImpl 的构造函数处理。
     */
    @Override
    public BufferPoolManager createBufferPoolManager(Config config, Logger logger) {
        // 直接返回实现类的实例，不要在这里写匿名类
